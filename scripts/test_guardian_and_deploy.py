...
def test_watchdog_deploy_window_and_flag() -> None:
    ...
    check("deploy window 15:44 ET blocked (still inside session)", w._deploy_window_allows(at(15, 44)) is False)
    check("deploy window 15:45 ET blocked (EOD close firing)", w._deploy_window_allows(at(15, 45)) is False)
    check("deploy window 15:46 ET allowed (after 15:45 EOD)", w._deploy_window_allows(at(15, 46)) is True)
