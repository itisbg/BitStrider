"""Self-check for the momentum-entry freshness guard (2026-08-11).

Reject a gap/momentum signal (PreMarketMomentum, GapBreakout) if price has
already faded off its recent high by the time the order is about to submit
-- different axis from the (disabled) GAP_CHASE guard, which gates on gap
SIZE. Case studies from 2026-08-11 (ACHR, PLUG, CLSK, SOUN) all faded before
or shortly after fill despite gaps ranging 5.5%-27.9%: magnitude didn't
predict the failure, timing did. See engine/config.py MOMENTUM_FRESHNESS_*
for the full reasoning and known limitations (catches sharp reversals like
PLUG's costlier 2nd entry, not gradual multi-hour bleeds like CLSK's).
...
fresh, reason = enhanced._check_momentum_freshness(_sig(strategy="VWAPFade"))
