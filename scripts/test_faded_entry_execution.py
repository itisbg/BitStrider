...
try:
    ...
    # Faded, past 90s, price fell further (mid now 2.80) -> re-chases uncapped-looking
    # quote, but still bounded (min with ceiling is moot here since 2.80*1.02=2.856 < 2.97).
    ...
    assert req.limit_price < 2.97, f"a continuing fade should chase BELOW the ceiling, got {req.limit_price}"

    # Faded, past 90s, price REVERSED UP (mid now 3.10) -> capped at ceiling 2.97, not chased up.
    ...
    ex._sweep_pending_entries()
    req = ex.client.submitted[-1]
    assert req.limit_price == 2.97, f"reversal must cap at the ceiling, not chase up to ~3.16, got {req.limit_price}"

    # Faded, past the 5-min ceiling timeout, price still up -> cap dropped, chases the real (higher) mid.
    ...
    ex._sweep_pending_entries()
    req = ex.client.submitted[-1]
    assert req.limit_price > 2.97, f"past the ceiling timeout should drop the cap and chase, got {req.limit_price}"

    # Non-faded entry: normal AFTERHOURS_CHASE_STALE_SECONDS threshold, no cap ever applied
    # (no price_ceiling key at all -- matches what _create_bracket_order stores for a
    # non-stale_entry signal).
    ...
    assert ex.client.submitted[-1].limit_price > 100.0, "normal (non-faded) chase is uncapped, marketable-side"
...
_orig_trail = ee._trail_pct_for
ee._trail_pct_for = lambda symbol, price, entry_log: (4.0, "NORMAL/THIN")  # stub out the ATR/bars lookup
try:
    ex = EnhancedExecutor.__new__(EnhancedExecutor)
