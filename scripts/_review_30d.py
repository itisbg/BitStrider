...
two-window trading schedule (2026-09-01): entries 09:14-11:00 + 14:45-15:50 ET,
lunch-flat at 11:00, EOD exit at 15:50, no overnight holds.
...
Each reconstructed leg is then checked against the new limits:
  ...
  - entry after 15:50 ET    -> BLOCKED (EOD close limit)
  - leg still open at 11:00 -> lunch-flat sweep would close it
  - leg still open past 15:50 / overnight -> EOD close sweep would close it
...
W_PM_OPEN, W_PM_CLOSE = 14 * 60 + 45, 15 * 60 + 50  # 14:45-15:50 ET (inclusive)
...
def main() -> None:
    ...
    for rt in viol["lunch"][:15]:
        print(f"  {rt['date']} {rt['sym']:<6} {rt['kind']:<9} {'short' if rt['dir']<0 else 'long':<5} "
              f"entry {rt['entry_et']} ET ${rt['pnl']:+.2f}")
    print(f"entries after 15:50 ET (would be blocked): {len(viol['after_pm'])}")
    ...
    print(f"positions still open at/after 15:50 (EOD close would cut): {len(held_eod)}")
